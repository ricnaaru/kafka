...
class ConsumerGroup {
  ...
  Future heartbeat(GroupSubscription subscription) async {
    ...
    var request = new HeartbeatRequest(name, subscription.generationId, subscription.memberId);
    _logger.fine('Sending heartbeat for member ${subscription.memberId} (generationId: ${subscription.generationId})');
    return session.send(request, host.host, host.port);
  }

  /// Retrieves offsets of this consumer group for specified [partitions].
  Future<List<ConsumerOffset>> fetchOffsets(List<TopicPartition> partitions) async {
  ...
  Future<List<ConsumerOffset>> _fetchOffsets(
    ...
    try {
      ...
      if (retries > 1) {
        _logger.info('GroupMember(${name}): encountered NotCoordinatorForConsumerError(16) when fetching offsets. '
      ...
      if (retries > 1) {
        _logger.info('GroupMember(${name}): encountered OffsetsLoadInProgressError(14) when fetching offsets. '
            'Scheduling retry after delay.');
        return new Future<List<ConsumerOffset>>.delayed(const Duration(seconds: 1), () async {
  ...
  Future commitOffsets(List<ConsumerOffset> offsets, {GroupSubscription? subscription}) {
  ...
  Future _commitOffsets(
    ...
    try {
      ...
      var request = new OffsetCommitRequest(name, offsets, generationId, memberId, retentionInMsecs);

      _logger.fine("commiting offsets: group_id: $name on $host member_id: $memberId");
      await session.send(request, host.host, host.port);
    } on NotCoordinatorForConsumerError {
      if (retries > 1) {
        _logger.info('ConsumerGroup(${name}): encountered NotCoordinatorForConsumerError(16) when commiting offsets. '
            'Scheduling retry with metadata refresh.');
        return _commitOffsets(offsets, subscription: subscription, retries: retries - 1, refresh: true);
      } else {
        rethrow;
      }
    }
  ...
  Future resetOffsetsToEarliest(List<TopicPartition> topicPartitions, {GroupSubscription? subscription}) async {
    ...
    for (var earliest in earliestOffsets) {
      ...
      offsets.add(new ConsumerOffset(earliest.topic, earliest.partition, actualOffset, 'resetToEarliest'));
    }
  ...
  Future<Broker> _getCoordinator({bool refresh = false}) {
    ...
    if (_coordinatorHost == null) {
      _coordinatorHost = session.metadata.fetchGroupCoordinator(name).catchError((error) {
        _coordinatorHost = null;
  ...
  Future<GroupSubscription> join(
    ...
    String memberId,
    String protocolType,
    Iterable<GroupProtocol> groupProtocols,
  ) async {
    ...
    var joinRequest = new JoinGroupRequest(
      ...
      memberId,
      protocolType,
      groupProtocols.toList(),
    );
    JoinGroupResponse joinResponse = await session.send(joinRequest, broker.host, broker.port);
    ...
    var syncRequest = new SyncGroupRequest(name, joinResponse.generationId, joinResponse.memberId, groupAssignments);
    ...
    try {
      ...
      return new GroupSubscription(joinResponse.memberId, joinResponse.leaderId, syncResponse.assignment,
          joinResponse.generationId, joinResponse.groupProtocol);
    } on RebalanceInProgressError {
      _logger.warning('Received "RebalanceInProgress" error code for SyncRequest, will attempt to rejoin again now.');
      return join(sessionTimeout, rebalanceTimeout, memberId, protocolType, groupProtocols);
    }
  }

  Future<List<GroupAssignment>> _assignPartitions(String protocol, JoinGroupResponse joinResponse) async {
    var groupAssignments = <GroupAssignment>[];
    ...
    var partitionsPerTopic =
        new Map<String, int>.fromIterable(meta.asList, key: (_) => _.name, value: (_) => _.partitions.length);

    Map<String, List<TopicPartition>> assignments = assignor.assign(partitionsPerTopic, subscriptions);
    for (var memberId in assignments.keys) {
      ...
      assignments[memberId]!.forEach((topicPartition) {
        partitionAssignment.putIfAbsent(topicPartition.topic, () => <int>[]);
        partitionAssignment[topicPartition.topic]!.add(topicPartition.partition);
      });
      groupAssignments.add(new GroupAssignment(memberId, new MemberAssignment(0, partitionAssignment, null)));
    }
...
class GroupSubscription {
  ...
  GroupSubscription(this.memberId, this.leaderId, this.assignment, this.generationId, this.groupProtocol);
